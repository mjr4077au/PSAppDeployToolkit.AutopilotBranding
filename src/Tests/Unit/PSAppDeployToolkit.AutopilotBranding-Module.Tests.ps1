BeforeAll {
    ...
    #-------------------------------------------------------------------------
    $ModuleName = 'PSAppDeployToolkit.AutopilotBranding'
...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module PSAppDeployToolkit.AutopilotBranding.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain PSAppDeployToolkit.AutopilotBranding.psm1' {
            $PathToManifest |
                Should -FileContentMatchExactly "PSAppDeployToolkit.AutopilotBranding.psm1"
        } #validPSM1
